"""Tests for TogetherAI reasoning model support."""
...
# standard model returns no chain-of-thought; DeepSeek-R1 does
CHAT_MODEL = "meta-llama/Llama-3.3-70B-Instruct-Turbo"
REASONER_MODEL = "deepseek-ai/DeepSeek-R1"
...
def test_generate_no_reasoning():
    ...
    """
    llm = TogetherAI_LLM(model=CHAT_MODEL)
...
def test_generate_with_reasoning_returns_tuples():
    ...
    """
    llm = TogetherAI_LLM(model=REASONER_MODEL, enable_reasoning=True)
    results = llm.generate(user=USER_PROMPT)
    ...
    # the reasoner model should produce chain-of-thought
...
def test_generate_non_reasoning_model_has_no_reasoning():
    ...
    """
    llm = TogetherAI_LLM(model=CHAT_MODEL, enable_reasoning=True)
    results = llm.generate(user=USER_PROMPT)
    ...
    assert isinstance(response, str)
    assert len(response) > 0
    # standard instruct models do not produce reasoning content
